...
import org.trvedata.sgm.AccountableDsgmClient;
 ...
 */
public class AccountablePrintingDsgmListener implements AccountableDsgmClient.DsgmListener {
    private final String myName;
    private final HashMap<IdentityKey, String> names;

    public AccountablePrintingDsgmListener(String myName, HashMap<IdentityKey, String> names) {
        this.myName = myName;
        this.names = names;
    }
    ...
    public void onIncomingMessage(IdentityKey sender, byte[] plaintext) {
        Logger.i("PrintingDcgkaListener", myName + ": Incoming message from " + names.get(sender) +
    ...
    public void onUpdate(IdentityKey updater, Object messageId) {
        ...
        Logger.i("PrintingDcgkaListener", myName + ": Update from " + names.get(updater) + messageIdString);
    }

    @Override
    public void onAdd(IdentityKey adder, IdentityKey added, Object messageId) {
        String messageIdString = (messageId instanceof MessageId) ? (", number: " + ((MessageId) messageId).number) : "";
        Logger.i("PrintingDcgkaListener", myName + ": Add from " + names.get(adder) +
    ...
    public void onRemove(IdentityKey remover, ArrayList<IdentityKey> removed, Object messageId) {
        ...
        removedString += "]";
        Logger.i("PrintingDcgkaListener", myName + ": Remove from " + names.get(remover) +
    ...
    public void onAck(IdentityKey acker, Object acked) {
        ...
        Logger.i("PrintingDcgkaListener", myName + ": Ack from " + names.get(acker) + messageIdString);
    }
}
